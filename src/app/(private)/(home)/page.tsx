  ...
  const fetchMessages = useCallback(async (channelId: string) => {
    ...
    if (error) {
     ...
     const formattedMessages: Message[] = data?.map((msg: any) => ({
         ...msg,
         user_metadata: msg.user?.user_metadata || null
     })) || [];
    ...
    if (existingChannel) {
      ...
      fetchChannels();
       ...
       const tempNewChannel: Channel = {
           ...
           members: [
               { user_id: currentUserId, user: { user_metadata: currentUser } },
               { user_id: targetUserId, user: { user_metadata: user } },
           ]
       };
    ...
    const subscription = supabase
      ...
      .on('postgres_changes', {
        ...
        filter: `channel_id=eq.${selectedChannel.id}`,
         ...
         supabase
           .from('messages')
           .select('*, user:sender_id (user_metadata)')
  ...
  return (
    <div className="flex items-center justify-center h-screen bg-[#111] text-white px-8 py-12">
      <div className="flex w-full max-w-screen-xl h-full bg-[#1a1a1a] rounded-lg shadow-lg overflow-hidden">
        <aside className="w-80 p-4 bg-[#2a2a2a] flex flex-col h-full flex-shrink-0"> {/* Add flex-shrink-0 */}
        ...
        <section className="flex-1 flex flex-col bg-[#222] rounded-r-lg h-full">
          <header className="flex items-center justify-between p-4 border-b border-gray-700 flex-shrink-0">
             <h2 className="text-xl font-semibold text-pink-500">
              {selectedChannel ? getChannelName(selectedChannel) : 'Selecione uma conversa'}
            </h2>
